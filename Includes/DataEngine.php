...
class DataEngine {
    ...
    public $compactPointFormat = TRUE;
...
//$localxAxisField = mysqli_field_name($result, 0);
            ...
            do {
                if (!empty($this->dateGrouping)) {
                    ...
                    $strJSON = rtrim($strJSON, ',');
                    $strJSON .= '},';
                } else if (!empty($this->splitByField)) {
                    if (empty($splitByValue)) {
                        $strJSONCompressed = '[';
                    }
                    $splitByValueCurrent = $row[$splitByColumn];
                    if ($splitByValue != $splitByValueCurrent) {//new series
                        if (!empty($splitByValue)) {//Added between each series
                            $strJSON = rtrim($strJSON, ',');
                            $strJSON .=']'; //end points
                            if ($this->compactPointFormat) {
                                $pointArray = json_decode($strJSON);
                                $pointsCompressed = $this->pointsToArray($pointArray);
                                $strJSONCompressed .= $pointsCompressed;
                            } else {
                                $strJSONCompressed .= $strJSON;
                            }
                            $strJSONCompressed .= '},';
                            $strJSON = '[';
                        }
                        $splitByValue = $splitByValueCurrent;
                        $strJSONCompressed .='{"name":"';
                        $strJSONCompressed .= $splitByValue;
                        $strJSONCompressed .='","points":';
                    }
                    ...
                    if ($yAxisColumn > -1) {
                        if ($yAxisFieldType == 12) {
                            $dtEnd = strtotime($row[$yAxisColumn]);
                            if ($yAxisStartColumn > -1) {
                                $dtStart = strtotime($row[$yAxisStartColumn]);
                                $strJSON .= '"y": [' . (date('U', $dtStart) * 1000) . "," . (date('U', $dtEnd) * 1000) . '],';
                            } else {
                                $strJSON .= '"y":' . (date('U', $dtEnd) * 1000) . ',';
                            }
                        } else {
                            if ($yAxisStartColumn > -1) {
                                $strJSON .= '"y": [' . $row[$yAxisStartColumn] . "," . $row[$yAxisColumn] . '],';
                            } else {
                                $strJSON .= '"y":' . $row[$yAxisColumn] . ',';
                            }
                        }
                    } else if ($yAxisColumn > -1) {
                        $strJSON .= '"y":' . $row[$yAxisColumn] . ',';
                    }
...
//intialize the YAxis list
    ...
    private function getDataField($name) {
        ...
        if ($i !== False) {
            ...
            } else {//this is last entry without "," in field's name
                $fieldValue = substr($this->dataFields, $i);
                $i = $i - $len;
    ...
    private function getFinancialSeries() {
        ...
        if ($this->dataFieldSet === FALSE) {
            ...
            if (isset($totalSeries) && $totalSeries > 0)
        ...
        if ($scCount > 0) {
            for ($s = 0; $s < count($sc); $s++) {
                ...
                for ($el = 0; $el < count($sc[$s]->Elements); $el++) {
                    ...
                    $dt2 = $elem->xValue;
                    if ($elem->volume) { //point object for NavigatorMultiY (seriestype: 'ohlc' doesn't work, 
                        //Array       
                        $this->compactPointFormat = FALSE;
    ...
    private function pointsToArray($points) {       
        $template = "{\"mapTo\":\"%FIELDS%\",\"data\":[%ARR%]}";       
        ...
        for ($i = 0, $iLen = count($points); $i < $iLen; $i++) {
            ...
            $tempValues = rtrim($tempValues, ',');
            array_push($pntArr, '[' . $tempValues . ']');            
        }
